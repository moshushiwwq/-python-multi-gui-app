...
class ToggleSwitch(Widget) :
    ...
    size = ListProperty([dp(300) , dp(36)])  # 整体尺寸（宽度决定中间空白）
    ...
    def __init__(self , **kwargs) :
        ...
        super().__init__(**kwargs)
        self.size_hint = (None , None)  # 禁用自动缩放
        ...
        # 右侧开关容器
        self.switch_container = Widget(
            size_hint = (None , None) ,
            size = (dp(50) , self.thumb_size[1])
        )
    ...
    def _update_positions(self , *args) :
        ...
        self.label.y = self.y + (self.height - self.label.height) / 2  # 垂直居中

        # 2. 开关容器定位（组件最右侧）
        self.switch_container.size = (dp(50) , self.thumb_size[1])
        self.switch_container.pos = (
            self.x + self.width - self.switch_container.width ,  # 组件内最右侧
            self.y + (self.height - self.switch_container.height) / 2  # 垂直居中
        )
        ...
        # 4. 滑块位置（根据状态切换）
        self.thumb.size = self.thumb_size
        if self.active :
            # 激活状态：滑块靠右
            self.thumb.pos = (
                self.switch_container.right - self.thumb_size[0] - dp(2) ,
                self.switch_container.y + (self.switch_container.height - self.thumb_size[1]) / 2
            )
        else :
            # 非激活状态：滑块靠左
            self.thumb.pos = (
                self.switch_container.x + dp(2) ,
                self.switch_container.y + (self.switch_container.height - self.thumb_size[1]) / 2
            )
